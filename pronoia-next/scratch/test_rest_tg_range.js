...
async function run() {
  // Query: profile.telegramId >= 0
  const rangeQueryNumber = {
    structuredQuery: {
  ...
  await test("Telegram ID (Number) >= 0", rangeQueryNumber);
  await test("Telegram ID (String) >= ''", rangeQueryString);
}
