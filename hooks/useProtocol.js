  ...
  for (let a = 0; a < anchors.length - 1; a++) {
    ...
    if (windowEndMin < windowStartMin) {
      if (windowEndMin < 360 && windowStartMin > 1080) {
        ...
        // And truncate the start anchor's end time to the end anchor's start time to resolve overlap!
        const endAnchorStart = parseTimeToMin(endAnchor.startTime);
        const startAnchorStart = parseTimeToMin(startAnchor.startTime);
        virtualBlocks[startAnchor.idx].calculatedEndMin = endAnchorStart;
        virtualBlocks[startAnchor.idx].virtualDuration = Math.max(0, endAnchorStart - startAnchorStart) * 60;
        windowEndMin = endAnchorStart;
      }
    }
