...
export function generateOptimizedDaySchedule(chronotype, wakeTimeStr, bedTimeStr, dayName, liabilitiesList, goals) {
  const [wh, wm] = (wakeTimeStr || "07:00").split(":").map(Number);
  ...
  addBlockIfFits(peaks.cog2.startMin, peaks.cog2.endMin, {
  ...
  addBlockIfFits(wakeMin, wakeMin + 60, {
  ...
  // 7. Add evening wind-down (bedTime - 60 to bedTime)
...
export function generateCompleteWeekCalendar(chronotype, wakeTimeStr, bedTimeStr, liabilitiesList, goals) {
  ...
  days.forEach((dayName, idx) => {
    ...
    weekCalendar[dateStr] = {
      blocks: generateOptimizedDaySchedule(chronotype, wakeTimeStr, bedTimeStr, dayName, liabilitiesList, goals)
    };
  });
