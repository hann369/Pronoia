...
export function generateOptimizedDaySchedule(chronotype, wakeTimeStr, bedTimeStr, dayName, liabilitiesList, goals, showerPreference = 'morning', shoppingPreference = 'weekly') {
  ...
  const hasLiabilityWithKeyword = (keywords) => {
    return dailyLiabilities.some(l => {
      const titleLower = (l.title || "").toLowerCase();
      return keywords.some(kw => titleLower.includes(kw));
    });
  };

  const hasManualBreakfast = hasLiabilityWithKeyword(['frühstück', 'breakfast', 'mahlzeit 1', 'mahlzeit i', 'fruehstueck']);
  const hasManualLunch = hasLiabilityWithKeyword(['mittagessen', 'lunch', 'mahlzeit 2', 'mahlzeit ii', 'mittag']);
  const hasManualDinner = hasLiabilityWithKeyword(['abendessen', 'dinner', 'mahlzeit 3', 'mahlzeit iii', 'abendbrot']);
  const hasManualSport = hasLiabilityWithKeyword(['sport', 'gym', 'workout', 'fitness', 'training', 'krafttraining']);
  const hasManualShower = hasLiabilityWithKeyword(['duschen', 'shower', 'bad', 'badewanne', 'hygiene']);
  const hasManualSleep = hasLiabilityWithKeyword(['schlaf', 'sleep', 'bett', 'bedtime', 'wind-down', 'schlafen']);
  ...
  if (hasSportToday) {
    ...
    if (showerPreference === 'sport' && !hasManualShower) {
  ...
  if (showerPreference === 'evening' && !hasManualShower) {
  ...
  if ((showerPreference === 'morning' || (showerPreference === 'sport' && !hasSportToday)) && !hasManualShower) {
