...
export async function verifyIdToken(authorizationHeader) {
  ...
  try {
    ...
    if (!projectId) {
      console.error("[serverAuth] verifyIdToken failed: NEXT_PUBLIC_FIREBASE_PROJECT_ID is not set");
      return null;
    }
    const { jwtVerify, createRemoteJWKSet } = await import('jose');
    const JWKS = createRemoteJWKSet(new URL('https://www.googleapis.com/service_accounts/v1/jwk/securetoken-system@system.gserviceaccount.com'));
