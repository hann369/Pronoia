  ...
  _registerBuiltins() {
    ...
    this.register({
      ...
      execute: async (actionId, params) => {
        console.log('[WHOOP Connector] Executing:', actionId, params);
        // Realistic simulation of wearable API request
        const mockHrv = Math.round(68 + Math.random() * 22);
        const mockSleep = Math.round(72 + Math.random() * 24);
        return {
          success: true,
          message: `WHOOP: Handshake abgeschlossen. HRV: ${mockHrv}ms, Sleep Score: ${mockSleep}%.`,
          data: { hrv: mockHrv, sleep: mockSleep }
        };
      }
    });
    ...
    this.register({
      ...
      execute: async (actionId, params) => {
        ...
        return {
          success: true,
          message: actionId === 'export_protocol'
            ? `Notion: Tagesprotokoll für ${params.date} (${params.blocksCount} Blöcke) erfolgreich übertragen.`
            : `Notion: "${params.title}" erfolgreich übertragen.`
        };
      }
    });
