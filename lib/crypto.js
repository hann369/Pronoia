...
// Encrypts plaintext directly for ONE recipient public key. Used by the
// per-message encryption model: no shared mutable group key, no healing,
// no key divergence — each message carries one cipher per participant.
export async function eciesEncryptText(plaintext, recipientPublicKey) {
  ...
  return {
    ...
    iv: arrayBufferToBase64(iv),
    ephemPub: await exportPublicKey(ephemKeyPair.publicKey)
  };
}

export async function eciesDecryptText(encObj, ownPrivateKey) {
  if (!isSubtleCryptoSupported()) throw new Error("SubtleCrypto not supported");
  if (!encObj?.ephemPub || !encObj?.ct || !encObj?.iv) throw new Error("Invalid cipher object");
